...
$aBook = Model::fromFragment('category',array( 'books' => array('authors','press') ) ) ;</code>
		...
		<p>向 PrototypeAssociationMap 类的单件实例中一次性添加整个系统所需要的数据表原型定义，然后就可以通过上面演示的方式，从中取出片段，并根据这些关系片段来创建Model对象。</p>
	</div>
	<blockquote>
		PrototypeAssociationMap 对象 add orm配置时，关联的 'prototype' 元素不再需要提供另一个数据表原型的完整定义，只要提供数据表原型的名称即可。
	</blockquote>
